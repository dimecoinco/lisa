...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #LisaCoinTEST2\r");
            Send(hSocket, "WHO #LisaCoinTEST2\r");
        } else {
            // randomly join #LisaCoin00-#LisaCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #LisaCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #LisaCoin%02d\r", channel_number).c_str());
        }
