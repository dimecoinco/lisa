LisaCoin-qt: Qt4 GUI for LisaCoin
===============================
...
Alternatively, install Qt Creator and open the `LisaCoin-qt.pro` file.

An executable named `LisaCoin-qt` will be built.
...
A warning for people using the *static binary* version of LisaCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of LisaCoin is linked against libdb5.3.
...
installed causes LisaCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
